...
function Main({userName, ChangeName, QuizLength, quizList}) {
  ...
  return (
    ...
    <div className='mx-auto basis-11/12 lg:basis-10/12'>
      ...
      <input defaultValue={userName} onChange={ChangeEvent} className='border pl-4 py-2 rounded-md shadow-md outline-none basis-full sm:basis-8/12' type="text" placeholder='이름을 입력해주세요'></input>
      ...
      <h3 className="my-5 basis-full text-center">{userName !=="" && <><span className='text-indigo-500 font-bold'>{userName}님</span><span>문제 유형은 기본값으로 설정되어있으며, 총{QuizLength}문제 중 1문제를 선택하셨습니다.</span></>} </h3>
      </div>
    </div>
